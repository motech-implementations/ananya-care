...
public abstract class CaseService<T> {
    ...
    @Autowired
    public void setResponseMessageBuilder(
    ...
    public ResponseEntity<String> processCase(HttpEntity<String> requestEntity)
            throws IOException {
        ...
        try {
            CommcareCaseParser<T> caseParser = new CommcareCaseParser<T>(clazz,
            ...
            logError(exception);
            return loggedResponse(new ResponseEntity<String>(
                    responseMessageBuilder.createResponseMessage(exception),
            ...
            return loggedResponse(new ResponseEntity<String>(
                    responseMessageBuilder.createResponseMessage(exception),
                    responseHeaders, exception.getHttpStatusCode()));

        } catch (RuntimeException exception) {
            logError(exception);
            return loggedResponse(new ResponseEntity<String>(
                    responseMessageBuilder.messageForRuntimeException(),
                    responseHeaders, HttpStatus.INTERNAL_SERVER_ERROR));
        }
        return loggedResponse(new ResponseEntity<String>(
                responseMessageBuilder.messageForSuccess(), responseHeaders,
                HttpStatus.OK));
    }

    private void logError(Throwable exception) {
        logger.error("Exception encountered while processing case xml"
                + exception.getMessage() + exception.getStackTrace(), exception);
    }

    private ResponseEntity<String> loggedResponse(
            ResponseEntity<String> responseEntity) {
        logger.info("Sending case xml Response: Status Code: "
                + responseEntity.getStatusCode() + "; Body: "
                + responseEntity.getBody());
        return responseEntity;
    }

    private void processCaseAction(CommcareCaseParser<T> caseParser, T object)
