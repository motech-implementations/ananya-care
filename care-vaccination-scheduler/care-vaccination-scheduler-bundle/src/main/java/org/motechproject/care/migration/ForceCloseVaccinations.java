...
public class ForceCloseVaccinations {
    ...
    public void closeCase(String caseId) {
        if (StringUtils.isNotEmpty(caseId)) {
            CareCaseTask careCaseTask = dbRepository.get(CareCaseTask.class,
                    "caseId", caseId);
            if (careCaseTask != null) {
                careCaseTaskService.close(
                        careCaseTask.getMotherCase() == null ? careCaseTask
                                .getChildCase() : careCaseTask.getMotherCase(),
                        careCaseTask.getMilestoneName());
                return;
            }
