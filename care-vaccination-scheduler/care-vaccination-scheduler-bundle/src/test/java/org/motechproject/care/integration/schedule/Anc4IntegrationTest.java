...
public class Anc4IntegrationTest extends SpringIntegrationTest {
	...
	@Before
	public void setUp() {
		List<VaccinationService> ancServices = Arrays
				.asList((VaccinationService) anc4Service);
		VaccinationProcessor motherVaccinationProcessor = new VaccinationProcessor(
				ancServices);
	...
	public void shouldVerifyAnc4ScheduleWithStartOfTrimesterWhenAnc3IsFulfilledMuchBefore() {
		...
		DateTime expectedReferenceDate = edd.minusDays(
				PeriodUtil.DAYS_IN_3RD_TRIMESTER).plus(
				periodUtil.getScheduleOffset());
		DateTime expectedStartDueDate = edd
				.minusDays(PeriodUtil.DAYS_IN_3RD_TRIMESTER);

		MotherCase mother = new MotherBuilder().withCaseId(caseId).withEdd(edd)
				.withANC1(null).withANC2(null).withANC3(null).withANC4(null)
				.build();
		motherService.process(mother);
		markScheduleForUnEnrollment(caseId, scheduleName);
		EnrollmentRecord enrollment;
		assertNull(getEnrollmentRecord(scheduleName, caseId,
				EnrollmentStatus.ACTIVE));

		mother = new MotherBuilder().withCaseId(caseId).withEdd(edd)
				.withANC1(anc1Date).withANC2(anc2Date).withANC3(anc3Date)
				.withANC4(null).build();
		motherService.process(mother);
		enrollment = getEnrollmentRecord(scheduleName, caseId,
				EnrollmentStatus.ACTIVE);
		assertEquals(MilestoneType.Anc4.toString(),
				enrollment.getCurrentMilestoneName());
		assertEquals(expectedReferenceDate, enrollment.getReferenceDateTime()
				.withTimeAtStartOfDay());
		assertEquals(expectedStartDueDate, enrollment.getStartOfDueWindow()
				.withTimeAtStartOfDay());
		assertEquals(edd.plusWeeks(2), enrollment.getStartOfLateWindow()
				.withTimeAtStartOfDay());
	}
	...
	public void shouldVerifyAnc4ScheduleWithAnc3DateWhenAnc3IsFulfilledInThirdTrimester() {
		...
		DateTime expectedReferenceDate = anc3Date.plusDays(30).plus(
				periodUtil.getScheduleOffset());
		DateTime expectedStartDueDate = anc3Date.plusDays(30);
		DateTime expectedStartLateDate = anc3Date.plusDays(30).plusDays(
				PeriodUtil.DAYS_IN_3RD_TRIMESTER);
		...
		motherService.process(mother);
		markScheduleForUnEnrollment(caseId, scheduleName);
		EnrollmentRecord enrollment = getEnrollmentRecord(scheduleName, caseId,
				EnrollmentStatus.ACTIVE);
		...
		assertEquals(expectedStartDueDate, enrollment.getStartOfDueWindow()
				.withTimeAtStartOfDay());
		assertEquals(expectedStartLateDate.plusWeeks(2), enrollment
				.getStartOfLateWindow().withTimeAtStartOfDay());
	}
