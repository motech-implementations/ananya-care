...
public class CommcareDao {
    ...
    public CommcareDao(QueryExecutor queryExecutor,
            @Qualifier("queryProperties") Properties queryProperties) {
    ...
    public Map<String, Object> getCases(CaseType caseType, String caseId) {
        if (caseType == caseType.Mother) {
            return queryExecutor.execute(SQL(FIND_QUERY_MOTHER_CASE),
                    buildParams(caseId));
        } else {
            return queryExecutor.execute(SQL(FIND_QUERY_CHILD_CASE),
                    buildParams(caseId));
        }
    }
