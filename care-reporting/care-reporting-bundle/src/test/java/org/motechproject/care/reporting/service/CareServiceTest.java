...
public class CareServiceTest {
	...
	public void shouldProcessAndSaveForms() {
		...
		motherFormValues.put("xmlns", "http://bihar.commcarehq.org/pregnancy/new");
	...
	public void shouldCloseCase() {
		MotherCase motherCase = new MotherCase();
		when(dbRepository.get(MotherCase.class, "caseId", "123")).thenReturn(motherCase);
		ChildCase childCase = new ChildCase();
		when(dbRepository.get(ChildCase.class, "caseId", "123")).thenReturn(childCase);
	...
	public void shouldSaveOrUpdateAllByExternalPrimaryKey() {
		...
		service.saveOrUpdateAllByExternalPrimaryKey(FlwGroup.class, Arrays.asList(updatedGroup));
		verify(dbRepository).saveOrUpdateAll(Matchers.anyListOf(Class.class));
	}
	
	@Test
	public void shouldGetLocationWithUnknownLocation() {
		when(dbRepository.get(any(Class.class), (Map)anyObject(), (Map)anyObject())).thenReturn(null);
		LocationDimension location = service.getLocation("bihar", "saharsa", "xyz");
	}
