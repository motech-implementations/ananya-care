...
public class MotherInfoParserTest {
    ...
    public void shouldMapCaseIdAndDateModified() throws Exception {
        ...
        FormValueElement motherCaseElement = new FormValueElementBuilder()
                ...
                .addAttribute("date_modified", dateModified).addAttribute(
                        "user_id", "89fda0284e008d2e0c980fb13fa0e5bb").build();

        FormValueElement childInfoElement = new FormValueElementBuilder()
                .addSubElement("hh_number", "555").addSubElement("age", "1")
                .build();

        final String receivedOn = DateTime.now().toString();
        CommcareForm commcareForm = new CommcareFormBuilder().withReceivedOn(
                receivedOn).addSubElement("hh_number", "165").addSubElement(
                "family_number", "5").addSubElement("case", motherCaseElement)
                .addSubElement("child_info", childInfoElement).build();

        when(infoParser.getCaseElement(commcareForm.getForm())).thenReturn(
                motherCaseElement);
    ...
    public void shouldReturnNullIfMotherCaseIsNotFoundAndLogError() {
        ...
        CommcareForm commcareForm = new CommcareFormBuilder().addSubElement(
                "hh_number", "165").addSubElement("family_number", "5").build();
    ...
    public void shouldThrowExceptionIfCaseIdIsEmpty() {
        ...
        CommcareForm commcareForm = new CommcareFormBuilder().addSubElement(
                "case", motherCaseElement).build();
        commcareForm.setId(instanceId);

        FormValueElement rootElement = commcareForm.getForm();
        when(infoParser.getCaseElement(rootElement)).thenReturn(
                motherCaseElement);
